// First run mongoimport -d pluralsight -c rent --file test.csv --type csv --headerline --drop
...
// to match with Text
// first create a index over any field as text search requires index on field
...
db.test.aggregate([
    {
        $project: {
            complete_name: {
            $concat: ["$name", " ", "$experiences_offered"]
            }
        }
    }
])
